...
class Splunk_TA_citrix_netscaler_rh_validate_input_configuration(admin.MConfigHandler):
    ...
    def check_duplication(self, input_name, appliances, templates):
        ...
        for task_input in input_objs_dict:
            if ("citrix_netscaler://" not in task_input) or task_input == input_name or utils.is_true(input_objs_dict.get(task_input, {}).get("disabled", "1")):
                continue
            for task_appliance in input_objs_dict.get(task_input, {}).get("servers", "").split("|"):
                for task_template in input_objs_dict.get(task_input, {}).get("templates", "").split("|"):
                    for endpoint in template_objs_dict.get(task_template, {}).get("content", "").split(";"):
                        api_endpoint = appliance_objs_dict.get(task_appliance, {}).get("server_url", "") + "``" + endpoint
                        if api_endpoint not in existing_endpoints:
                            existing_endpoints[api_endpoint] = (endpoint, task_appliance, task_template, task_input)
        
        for appliance in appliances.split("|"):
            for template in templates.split("|"):
                unique_endpoint = False
                for endpoint in template_objs_dict.get(template, {}).get("content", "").split(";"):
                    api_endpoint = appliance_objs_dict.get(appliance, {}).get("server_url", "") + "``" + endpoint
                    if api_endpoint not in existing_endpoints:
                        existing_endpoints[api_endpoint] = (endpoint, appliance, template, input_name)
                        ...
                        "which may cause data duplication.".format(
                            input_name.replace("citrix_netscaler://", ""), template,
                            appliance_objs_dict.get(appliance, {}).get("server_url", ""),
                            endpoint, existing_endpoints[api_endpoint][3].replace("citrix_netscaler://", ""),
                            existing_endpoints[api_endpoint][2], appliance_objs_dict.get(existing_endpoints[api_endpoint][1], {}).get("server_url", "")))
                ...
                if not unique_endpoint:
                    logger.warn("All api endpoints in [ task=({}) , template=({}), appliance url=({}) ] "
                            ...
                            input_name.replace("citrix_netscaler://", ""), template, appliance_objs_dict.get(appliance, {}).get("server_url", "")))
