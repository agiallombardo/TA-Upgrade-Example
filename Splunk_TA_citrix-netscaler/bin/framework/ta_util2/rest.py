...
def build_http_connection(config, timeout=120):
    ...
    disable_ssl_certificate_validation = utils.is_true(config.get("disable_ssl_certificate_validation"))
    ca_certs_path = config.get("ca_certs_path")

    if ca_certs_path:
        ca_certs_path = ca_certs_path.strip()

    _LOGGER.debug("Found %s value for disable_ssl_certificate_validation parameter",
    ...
    http = httplib2.Http(proxy_info=proxy_info, timeout=timeout,
                         disable_ssl_certificate_validation=disable_ssl_certificate_validation, ca_certs=ca_certs_path)
