...
class TAConfTask:
    ...
    def encrypt_credentials(self):
        ...
        server_file_path = os.path.join(splunk_home, "etc", "apps", app_name, "local", self.server_file)
        ...
        if op.isfile(server_file_path):
            ...
            conf_mgr = conf_manager.ConfManager(self.metas["session_key"], self.metas["app_name"],
            ...
            try:
                server_conf = conf_mgr.get_conf(self.server_file.replace(".conf", ""), refresh=True)
            except conf_manager.ConfManagerException:
                _LOGGER.debug("Config file: {} not found. So, exiting modular input.".format(self.server_file))
                return
            server_conf.reload()
            ...
            try:
                ...
                raise Exception("It looks like there has been unexpected modification in: {} file from backend, Data collection for all inputs have been stopped. Please reload the configuration page from UI or restart Splunk. Exiting..".format(self.server_file))
            ...
            for server in server_config:
                ...
                if (server_info.get("account_name") is None and server_info.get("account_password") is None) or (server_info.get("account_name") == "" and server_info.get("account_password") == ""):
                    continue
                ...
                if (server_info["account_name"] != ENCRYPTED_MAGIC_TOKEN and server_info[
                    "account_password"] != ENCRYPTED_MAGIC_TOKEN):
                    _LOGGER.info("Encrypting clear username and password for server=%s",
                                 server_info["server_url"])
                    server_conf.update(server, {"account_name": server_info["account_name"],
                ...
                ## Check if only appliance username is in clear text and encrypt accordingly
                    ...
                    _LOGGER.info("Encrypting clear username for server=%s",
                                 server_info["server_url"])
                    server_conf.update(server, {"account_name": server_info["account_name"],
                ...
                ## Check if only appliance password is in clear text and encrypt accordingly
                    ...
                    _LOGGER.info("Encrypting clear password for server=%s",
                                 server_info["server_url"])
                    server_conf.update(server, {"account_name": server_stanza_user,
                                                "account_password": server_info["account_password"]},
                                       ["account_name", "account_password"])

        global_file_path = os.path.join(splunk_home, "etc", "apps", app_name, "local", self.global_file)

        if op.isfile(global_file_path):
            global_config = cli.readConfFile(global_file_path)
            global_config.pop('default')

            conf_mgr_global = conf_manager.ConfManager(self.metas["session_key"], self.metas["app_name"],
            ...
            try:
                global_conf = conf_mgr_global.get_conf(self.global_file.replace(".conf", ""), refresh=True)
            except conf_manager.ConfManagerException:
                _LOGGER.debug("Config file: {} not found. So, exiting modular input.".format(self.global_file))
                return
            ...
            for stanza in global_config:
                if stanza == "proxy":
                    ...
                    if proxy_password != None and proxy_password != "" and proxy_password != ENCRYPTED_MAGIC_TOKEN:
                        _LOGGER.info("Encrypting clear password for proxy_server=%s",
                                     proxy_info["proxy_url"])
                        global_conf.update(stanza, {"proxy_password": proxy_password},
                                           ["proxy_password"])
                    break
    ...
    def get_tasks(self, inputs):
        ...
        for k, v in conf_objs[-2].items():
            ...
            if "disable_ssl_certificate_validation" in v:
                disable_ssl_certificate_validation = v["disable_ssl_certificate_validation"]
        ...
        for task_config in all_task_configs:
            ...
            task_config["disable_ssl_certificate_validation"] = disable_ssl_certificate_validation
    ...
    def _create_task_config(task, task_template, remote_server):
        ...
        required_keys = ("account_name", "account_password", "server_url",
                         "content", "index", "duration", "disabled")
        ...
        for server_stanza in ns_servers:
            if server_stanza in remote_servers:
                ...
                msg = ("{} server is referenced by tasks.conf, but it is "
                       "disabled or deleted in servers.conf.").format(
                    server_stanza)
                _LOGGER.error(msg)
        return task_configs
    ...
    def _get_conf_objects(ConfMgr, file_name):
        ...
        try:
            stanzas = ConfMgr.get_conf("{}".format(file_name.replace(".conf", ""))).get_all(only_current_app=True)
        except conf_manager.ConfManagerException:
            return {}
