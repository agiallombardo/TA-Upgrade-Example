...
class CitrixNetscalerConfig:
    ...
    def get_tasks(self, inputs, ui_input_validator_args=None, ui_input_validator_logger=None):
        """
        ...
        if ui_input_validator_args:
            tasks, ui_task_objs = self._separate_tasks(tasks, ui_input_validator_args[0])
        ...
        for task in tasks:
            ...
            if not content:
                _LOGGER.warn("No api_endpoint specified for task={}, ignoring it".format(task["name"].replace("citrix_netscaler://","")))
                continue
            existing_endpoints, api_endpoints, is_ui_input_warning = self._process_api_endpoints(task, content,
                                                                                                 existing_endpoints,
                                                                                                 ui_logger=None)
        ...
        if ui_input_validator_args:
            ...
            for task in ui_task_objs:
                ...
                if not ui_task_content:
                    _LOGGER.warn("No api_endpoint specified for task={}, ignore it".format(task["name"].replace("citrix_netscaler://","")))
                    continue
                existing_endpoints, api_endpoints, is_ui_input_warning = self._process_api_endpoints(
                                                    task, ui_task_content, existing_endpoints,
    ...
    def _process_api_endpoints(self, task, content, existing_endpoints, ui_logger=None):
        ...
        for endpoint in content.split(";"):
            ...
            if et not in existing_endpoints:
                existing_endpoints[et] = (endpoint, task["server_url"], task["task_template"], task["name"])
                ...
                if ui_logger:
                    ...
                    ui_logger.warn(
                        ...
                        "which may cause data duplication.".format(
                            task["name"].replace("citrix_netscaler://",""), task["task_template"].replace(self.metas["app_name"] + ":",""),
                            task["server_url"], endpoint, existing_endpoints[et][3].replace("citrix_netscaler://",""),
                            existing_endpoints[et][2].replace(self.metas["app_name"] + ":",""), existing_endpoints[et][1])
                    )
                    is_ui_input_warning = True
                else:
                    _LOGGER.warn("While filtering [ task=({}), template=({}), appliance url=({}) ] found "
                                 "api_endpoint=({}) already specified in "
                                 "[ task=({}), template=({}), appliance url=({}) will have data duplication."
                                 "".format(task["name"].replace("citrix_netscaler://",""), task["task_template"].replace(self.metas["app_name"] + ":",""), task["server_url"],
                                                      endpoint, existing_endpoints[et][3].replace("citrix_netscaler://",""),
                                                      existing_endpoints[et][2].replace(self.metas["app_name"] + ":",""), existing_endpoints[et][1]))

            api_endpoints.append(endpoint)
        if ui_logger:
            self._generate_all_duplicate_endpoints_warning(task, unique_api_endpoints, ui_logger)
        else:
            self._generate_all_duplicate_endpoints_warning(task, unique_api_endpoints, _LOGGER)
    ...
    def _generate_all_duplicate_endpoints_warning(self, task, api_endpoints, logger=None):
        """
        ...
        if not api_endpoints:
            logger.warn("All api endpoints in [ task=({}) , template=({}), appliance url=({}) ] "
                        "duplicate with other tasks.".format(
                        task["name"].replace("citrix_netscaler://",""), task["task_template"].replace(self.metas["app_name"] + ":",""), task["server_url"]))
