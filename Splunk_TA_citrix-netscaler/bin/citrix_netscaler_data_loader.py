...
def _convert_to_modinput_format(content, resource_name,
                                api_endpoint, url, index, host, input_name):
    """
    response format:
        {
    ...
    evt_fmt = ("<event stanza=\"{0}\"><source>{1}</source><sourcetype>{2}</sourcetype>"
    ...
    results = (evt_fmt.format(input_name, source, sourcetype, host, index, rec)
...
class CitrixNetscaler(object):
    ...
    def _get_full_url(self, config):
        ...
        if config.get("resource_name"):
            ...
            full_url = "{}/nitro/{}/{}/{}".format(
                url, api_version,
                config["api_endpoint"], config["resource_name"])
        else:
            full_url = "{}/nitro/{}/{}".format(
                url, api_version, config["api_endpoint"])
        return full_url
    ...
    def _do_collect(self):
        ...
        if content is not None:
            params = (content, self._config.get("resource_name"),
                      self._config["api_endpoint"], self._config["url"],
                      self._config.get("index", "main"), self._config["host"], self._config["name"])
