...
basestring = basestring if sys.version_info < (3,0) else str
...
def _wrap_method(method, wsdl_name, arg_processor, result_processor, usage):
    ...
    value = method.method.name.encode("utf-8")
    wrapped_method.__name__ = value if sys.version_info < (3, 0) else value.decode("utf-8")
...
class _NativeResultProcessor(_ResultProcessor):
    ...
    def _convert_to_native_type(self, value):
        ...
        int_type = long if sys.version_info < (3, 0) else int
        if isinstance(value, list):
            ...
            # unicode.
            return str(value)
        elif isinstance(value, int_type):
