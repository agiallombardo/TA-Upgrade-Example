...
class F5BigIPPassiveServerLoader(object):
    ...
    def refresh(self):
        ...
        for (url, partitions, username, password, interval) in self.servers:
            try:
                ...
                logger.error(("failed to add server data with url:%s - Traceback:\n"%url)+traceback.format_exc())
    ...
    def update(self, server_models, template_models, global_interval):
        self.template = '\n'.join([t.content if isinstance(t, str) else t.content.decode() for t in template_models])
    ...
    def run(self):
        try:
            ...
            for (task, server) in task_generator:
                logger.debug('new work generated, server: %s',server.server_data.url )
                if self.stop.isSet():
                    break
                if task.start():
                    F5BigIPWorker(task, server, self.event_writer, self.stop).start()
                    logger.debug('worker started, server: %s',server.server_data.url )
                logger.debug('start to generete new work')
        except Exception as e:
            logger.error("Error in F5BigIPScheduler.run() - Traceback:\n"+traceback.format_exc())
...
class F5BigIPWorker(threading.Thread):
    ...
    def run(self):
        try:
            while True:
                for query in self.task.queries:
                    if self.server.to_be_deleted or self.stop.isSet():
                        self.task.terminate()
                        break
                    logger.debug('start fetch, server: %s',self.server.server_data.url)
                    items=query.fetch()
            ...
            logger.error("Error in F5BigIPWorker.run() - Traceback:\n"+traceback.format_exc())
            self.task.terminate()
    ...
    def json_event(self,item,timestamp):
        ...
        event=Event(source=source , index=index, sourcetype=sourcetype, host=source, time=timestamp, data=data)
        self.event_writer.write_event(event)
        logger.debug('ending index.')
