...
class F5TaskModel(SOLNAppObjModel):
    ...
    def get_server_keys(self):
        return [self.namespace +":"+ key.strip() for key in str(self.servers).split('|')] if self.servers else []

    def get_template_keys(self):
        return [self.namespace +":"+ key.strip() for key in str(self.templates).split('|')] if self.templates else []
        ...
        class AccessTaskIterator(object):
            ...
            def __iter__(self):
                for task in self.tasks:
                    try:
                        if (task.name).split("://")[0]== "f5_task":
                            task.name = (task.name).split("://")[1]
                            yield task
