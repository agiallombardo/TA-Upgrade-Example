...
from import_declare_test import ta_name, templates_conf
import logging
util.remove_http_proxy_env_vars()
logger = setup_logging("splunk_ta_f5_template")
...
class F5TemplateHandler(AdminExternalHandler):
    ...
    def handleList(self, conf_info):
        """
        This method is called when template is rendered with configurable parameters from splunk_ta_f5_bigip_template.conf
    ...
    def handle_request(self, conf_info, is_handle_edit_operation):
        ...
        try:
            ...
            common_utility.perform_validation(self.callerArgs.id, self.payload, common_utility.params.get('F5_BIGIP_TEMPLATES_CONF_FILE_NAME'), endpoint, is_handle_edit_operation, self.getSessionKey())
            # Gives the content of the template
            content = self.payload[common_utility.params.get('CONTENT_PARAM')]
            self.payload['content'] = str((common_utility.encode(self.payload['content'])).decode("utf-8"))
    ...
    def handleRemove(self, confInfo):
        ...
        :return: None
        """
        default_templates = ['Standard_GlobalLB', 'Standard_GlobalLB_v12_0_0', 'Standard_LocalLB', 'Standard_Management', 'Standard_Network', 'Standard_System']
        # This checks if template to be deleted is default template or not. If default template is attempted to be deleted, it raises an RestError
        if self.callerArgs.id in default_templates:
            raise RestError(405, "\"{}\" cannot be deleted because it is default template.".format(self.callerArgs.id))
        is_input_exist, field_name = common_utility.is_input_exists(self.callerArgs.id, self.getSessionKey(), "templates")
        if is_input_exist:
            raise RestError(405, "\"{}\" cannot be deleted because it is used in inputs.".format(field_name))
